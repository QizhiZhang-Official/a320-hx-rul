...
def load_an_aircraft(craft_no: str) -> list[dict]:
    ...
    if craft_no in os.listdir(DIR_2023):
        for file_name in os.listdir(os.path.join(DIR_2023, craft_no)):
            flight_data = {
                'file_name': file_name,
                'data': pd.read_csv(os.path.join(DIR_2023, craft_no, file_name))
            }
        craft_data.append(flight_data)
    
    if craft_no in os.listdir(DIR_2024):
        for file_name in os.listdir(os.path.join(DIR_2024, craft_no)):
            flight_data = {
                'file_name': file_name,
                'data': pd.read_csv(os.path.join(DIR_2024, craft_no, file_name))
            }
        craft_data.append(flight_data)
